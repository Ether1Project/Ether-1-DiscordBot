...
module.exports.run = async (bot, message, args) => {
  ...
  let msg = await message.channel.send("Vote now! (20 Minuts)");
  await msg.react(agree);
  await msg.react(disagree);

  const reactions = await msg.awaitReactions(reaction => reaction.emoji.name === agree || reaction.emoji.name === disagree, {time: 1200000});
  msg.delete();
  ...
  if(YES_Count >= 10 && YES_Count > NO_Count){
